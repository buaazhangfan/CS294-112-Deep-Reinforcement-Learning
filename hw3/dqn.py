...
class QLearner(object):
  ...
  def __init__(
    ...
    # The output Q-value from Q-function network and target network
    q_t = q_func(obs_t_float, self.num_actions, scope = "q_func", reuse = False)
    q_tp1 = q_func(obs_tp1_float, self.num_actions, scope = "target_func", reuse = False)

    #print(q_tp1.shape)
    self.best_action = tf.argmax(q_t, axis = 1)
    
    ''' Target Q-value(label)'''
  ...
  def log_progress(self):
    ...
    if self.t % self.log_every_n_steps == 0 and self.model_initialized:
      ...
      #Timestep.append(self.t)
      #mean_reward.append(self.mean_episode_reward)
      #best_reward.append(self.best_mean_episode_reward)
      #print(self.timestep_log)
      #print(self.mean_reward_log)
      #print(self.best_reward_log)

      #with open(self.rew_file, 'wb') as f:
      #  pickle.dump(episode_rewards, f, pickle.HIGHEST_PROTOCOL)
      data = {'Timestep': np.array(self.timestep_log),
