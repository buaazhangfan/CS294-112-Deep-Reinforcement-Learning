...
class Agent(object):
    ...
    def estimate_advantage(self, ob_no, next_ob_no, re_n, terminal_n):
        ...
        #raise NotImplementedError
        Value_t = self.sess.run(self.critic_prediction, feed_dict = {self.sy_ob_no: ob_no})
        Value_t1_temp = self.sess.run(self.critic_prediction, feed_dict = {self.sy_ob_no: next_ob_no})
        Value_t1 = (1 - terminal_n) * Value_t1_temp
        adv_n = re_n + self.gamma * Value_t1 - Value_t

        if self.normalize_advantages:
            #raise NotImplementedError
            _mean = np.mean(adv_n, axis = 0)
            _std = np.std(adv_n, axis = 0)
            adv_n = (adv_n - _mean) / _std
            # YOUR_HW2 CODE_HERE
        return adv_n
    ...
    def update_critic(self, ob_no, next_ob_no, re_n, terminal_n):
        ...
        for i in range(self.num_target_updates):
            ...
            Value_t1_temp = self.sess.run(self.critic_prediction, feed_dict = {self.sy_ob_no: next_ob_no})
            Value_t1 = (1 - terminal_n) * Value_t1_temp
            target = re_n + self.gamma * Value_t1
